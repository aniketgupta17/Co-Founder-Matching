...
def get_conversations():
    ...
    try:
        ...
        for conversation in conversations:
            ...
            other_user_id = conversation['user_id_1'] if conversation['user_id_2'] == user_id else conversation['user_id_2']
            
            # Get the other user's profile
            try:
                other_user_profile = SupabaseService.get_profile(other_user_id)
                other_user_name = other_user_profile.get('name', 'Unknown User') if other_user_profile else 'Unknown User'
                other_user_avatar = other_user_profile.get('avatar_url') if other_user_profile else None
            except Exception:
            ...
            enhanced_conversations.append({
                ...
                'latest_message': latest_message.get('content') if latest_message else None,
                'latest_message_time': latest_message.get('created_at') if latest_message else None,
                'unread_count': SupabaseService.get_unread_count(conversation['id'], user_id)
            })
...
def create_conversation():
    ...
    try:
        ...
        existing_conversation = SupabaseService.find_conversation(user_id, other_user_id)
        ...
        try:
            current_app.logger.info(f"Creating conversation between {user_id} and {other_user_id}")
            ...
            current_app.logger.error(f"Error in first attempt to create conversation: {str(e)}")
...
def send_message(conversation_id):
    ...
    try:
        ...
        receiver_id = conversation['user_id_1'] if conversation['user_id_2'] == user_id else conversation['user_id_2']
        ...
        try:
            current_app.logger.info(f"Sending message in conversation: {conversation_id}")
            ...
            current_app.logger.error(f"Error in first attempt to create message: {str(e)}")
...
def ensure_chat_tables():
    ...
    try:
        ...
        try:
            client.postgrest.rpc('exec_sql', {'sql': create_conversations_table}).execute()

...
class MatchingService:
    ...
    def generate_matches_for_user(self, user_id, top_n=10, exclude_user_ids=None):
        ...
        try:
            ...
            if exclude_user_ids is None:
                ...
                exclude_user_ids = set(
                    exclude_user_ids
                )  # Ensure it's a set for O(1) lookups

            current_app.logger.info(
                f"Excluding {len(exclude_user_ids)} users from matching"
            )

            # Get all users from Supabase
            all_users = SupabaseService.get_users()
            current_app.logger.error(f"Users: {len(all_users)}")
            ...
            current_app.logger.error(f"Profiles: {len(all_profiles)}")

            # Create a map of user_id to profile for quick lookup
            profile_map = {
                profile.get("user_id", profile.get("id")): profile
                for profile in all_profiles
                if profile.get("user_id") or profile.get("id")
            }
            ...
            current_app.logger.error(f"Existing matches: {len(existing_matches)}")
            ...
            for profile in all_profiles:
                ...
                if profile_id != user_id and profile_id not in exclude_user_ids:
                    potential_matches.append(profile)

            # for user in all_users:
            #     other_id = user.get("id")
            #     if other_id != user_id and other_id not in exclude_user_ids:
            ...
            current_app.logger.error(
                f"Found {len(potential_matches)} potential matches for user {user_id}"
            )
            ...
            #     subs = self._compute_subscores(target_user_profile, other_profile)
            #     score_val = self._weighted_score(subs)
            #     current_app.logger.error(f"Potential User: {other_user}")
            #     current_app.logger.error(f"Score: {score_val}")
            ...
            current_app.logger.info(
                f"Generated {len(top_matches)} top matches for user {user_id}"
            )
            return top_matches
        except Exception as e:
            current_app.logger.error(
                f"Error generating matches for user {user_id}: {str(e)}"
            )
            return []
    ...
    def generate_matches_for_all_users(self, top_n=10):
        ...
        try:
            ...
            current_app.logger.info(
                f"Found {len(users_with_profiles)} users with profiles out of {len(users)} total users"
            )
            ...
            for userA in users_with_profiles:
                ...
                for userB in users_with_profiles:
                    ...
                    if score_val >= 0.5:
                        explanation = self._generate_explanation(
                            profileA, profileB, subs, score_val
                        )
            ...
            for uid in recs_dict:
                # Sort descending
                sorted_list = sorted(
                    recs_dict[uid], key=lambda x: x["score"], reverse=True
                )
            ...
            current_app.logger.error(
                f"Error generating matches for all users: {str(e)}"
            )
            return {}
    ...
    def _generate_explanation(self, userA, userB, subscores, final_score):
        ...
        if cstyle_val > 0:
            lines.append(
                f"Your collaboration styles ({userA.get('collab_style')} and {userB.get('collab_style')}) work well together."
            )
